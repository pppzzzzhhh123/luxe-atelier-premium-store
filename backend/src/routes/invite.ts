...
router.get('/stats', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 获取邀请人数
    const { data: inviteRecords, count: inviteCount } = await supabase
      .from('invite_records')
      .select('*', { count: 'exact' })
      .eq('inviter_id', userId);

    // 获取返现记录
    const { data: rewardRecords } = await supabase
      .from('reward_records')
      .select('*')
      .eq('user_id', userId);
...
router.post('/withdraw', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 计算总金额
    const totalAmount = availableRewards.reduce(
      (sum, record) => sum + parseFloat(record.reward_amount),
      0
    );
