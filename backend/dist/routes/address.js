...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const { data: addresses, error } = await supabase_1.supabase
            .from('addresses')
            .select('*')
            .eq('user_id', userId)
...
router.get('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        const { data: address, error } = await supabase_1.supabase
            .from('addresses')
            .select('*')
            .eq('id', id)
            .eq('user_id', userId)
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { name, phone, province, city, district, detail, tag, isDefault = false, } = req.body;
        // 验证必填字段
...
router.put('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        // 检查地址是否存在
        const { data: existingAddress } = await supabase_1.supabase
            .from('addresses')
            .select('*')
            .eq('id', id)
            .eq('user_id', userId)
 ...
 * 设置默认地址
 */
router.post('/:id/default', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
        // 取消其他默认地址
